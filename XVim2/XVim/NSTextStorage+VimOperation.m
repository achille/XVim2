...
- (BOOL)isKeyword:(NSUInteger)index { return isKeyword([[self xvim_string] characterAtIndex:index]); }
...
/*
 ...
 Note: if MOPT_PARA_BOUND_BLANKLINE is passed in then blank lines with whitespace are paragraph boundaries. This is to
 get propper function for the delete a paragraph command(dap).
...
{ //(
    ...
    for (; pos < s.length && NSNotFound == paragraph_head; pos++, prevpos++) {
        ...
        if (isNewline(prevc) && !isNewline(c)) {
            if ([self xvim_nextNonblankInLineAtIndex:pos allowEOL:NO] == NSNotFound
                && opt == MOPT_PARA_BOUND_BLANKLINE) {
                paragraph_found++;
...
- (NSRange)searchRegexForward:(NSString*)pattern
...
- (NSRange)searchRegexBackward:(NSString*)pattern
                           from:(NSUInteger)index
                          count:(NSUInteger)count
                         option:(MOTION_OPTION)opt
{
...
{
    while (index > 0) {
        if (isNewline([string characterAtIndex:index - 1])) {
            break;
        }
        --index;
    }
...
int findmatchlimit(NSString* string, NSInteger pos, unichar initc, BOOL cpo_match)
{
    ...
    while (YES) {
        ...
        if (cpo_match) {
            ...
            NSInteger ptr = pos;
            while (ptr > 0 && !isNewline([string characterAtIndex:ptr - 1])) {
                --ptr;
            }
            NSInteger sta = ptr;

            while (ptr < maxIndex && !isNewline(characterAtIndex(h, ptr))) {
                if (ptr == pos + backwards) {
                    at_start = (do_quotes & 1);
                }

                if (characterAtIndex(h, ptr) == '"'
                    && (ptr == sta || characterAtIndex(h, ptr - 1) != '\'' || characterAtIndex(h, ptr + 1) != '\'')) {
                    ++do_quotes;
                }

                if (characterAtIndex(h, ptr) == '\\' && ptr + 1 < maxIndex
            ...
            if (!cpo_match && initc != '\'' && findc != '\'') {
                if (backwards) {
                    ...
                    if (col > 1) {
                        ...
                        else if (col > 2 && characterAtIndex(h, pos - 2) == '\\'
                    ...
                    if (pos < maxIndex && !isNewline(characterAtIndex(h, pos + 1))) {
                        if (characterAtIndex(h, pos + 1) == '\\' && (pos < maxIndex - 2)
                            && !isNewline(characterAtIndex(h, pos + 2)) && characterAtIndex(h, pos + 3) == '\'') {
                            pos += 3;
                            break;
                        }
                        else if (pos < maxIndex - 1 && characterAtIndex(h, pos + 2) == '\'') {
                            pos += 2;
                            break;
                        }
                    }
        ...
        default:
            ...
            if ((!inquote || start_in_quotes == YES) && (c == initc || c == findc)) {
                int bslcnt = 0;
...
- (NSUInteger)findForwardCharacterSet:(NSCharacterSet*)charSet
...
- (NSUInteger)findBackwardsCharacterSet:(NSCharacterSet*)charSet
                               beginPos:(NSUInteger)beginPos
                             delimiters:(NSCharacterSet*)delimiters
{
    NSString* string = [self xvim_string];
